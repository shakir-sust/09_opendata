...
One option is "Growing degree days"
...
fe_month <- fieldweather %>%
  ...
  dplyr::select(year, site, lat, lon,
                ...
                prcp.mm = prcp_mm_day, #to rename variable name
                srad.wm2 = srad_w_m_2,#to rename variable name
                tmax.c = tmax_deg_c, #to rename variable name
                tmin.c = tmin_deg_c,#to rename variable name
                vp.pa = vp_pa #to rename variable name
                ) %>%
  # Creating a date class variable  
  mutate(date_chr = paste0(year, "/", yday)) %>% #The format of "date_chr" is "chr", we will change it to "date" format
  mutate(date = as.Date(date_chr, "%Y/%j")) %>%
  # Extracting month from date  
  mutate(month = month(date)) %>% #month is in "dbl" (double) format
  mutate(month_abb = month(date, label = T)) #month_abb is in "ord" (ordinal) format
...
#8,376 rows = 698 site-years x 12 months
...
fe_month_sum_wide <- fe_month_sum %>%
  ...
  dplyr::select(-name, -month_abb) %>% #To remove "name" and "month_abb" columns so that we do not get any "NA"
  pivot_wider(names_from = varname,
              values_from = value) %>%
  # Rounding to one decimal point
  mutate(across(c(3:75), ~round(., 1) )) #alternatively: mutate(across(c(3:ncol(.)), ~round(., 1) )) #across(): to select the columns from 3 to 75 to perform rounding across these columns #round each of the column to 1 decimal place
...
ggplot(data = fe_month_sum,
  ...
  geom_density_ridges_gradient(scale = 3,
                               rel_min_height = 0.01) + #cuts down the tails of the distribution
  scale_fill_viridis_c(option = "C") + #viridis: color-blind-friendly color scale
  theme(legend.position = "none") #we use theme() to remove legends
...
finalplots <- fe_month_sum %>%
  ...
  labs(x = .y) # .y is the placeholder for map2() function
                     )) 
